        ...
        if user:
            if user.division:
                ...
                create_auth_event(user_data.get('GUID'), auth_event_type.USER_FAILED_LOG_IN,
                                  {'User Data': user_data, 'message': 'Awaiting division assignment.', 'success': False})
...
def _process_user_data(user_data: dict):
    ...
    if domain == 'records.nyc.gov':
        ...
        if user:
            ...
            user = _create_agency_user(User(**user_attributes))
        return user


def _create_agency_user(user):
    """
    Creates an agency user, logs an authentication event, and updates the user's
    role, login status, and last sign-in timestamp.

    Parameters:
    - user: User instance to be updated and persisted.
    - db_session: Database session for committing the changes.

    Returns:
    - User: The updated and persisted User instance.
    """
...
def _update_user_data(user: User, user_attributes: dict):
    """
